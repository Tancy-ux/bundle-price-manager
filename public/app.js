...
function App() {
  ...
  useEffect(() => {
    ...
    fetch("/api/reorder").then(r => r.json()).then(d => d && d.items && setReorder(d)).catch(() => {});
  }, []);
  ...
  }), tab === "reorder" && /*#__PURE__*/React.createElement(Reorder, {
    doc: reorder,
    setDoc: setReorder,
    stickyTop: headerH,
    flash: flash,
...
function Reorder({
  ...
  const shown = useMemo(() => {
    const list = rows.filter(r => inView(r) && (vendor === "all" || (r.vendor || "No vendor") === vendor) && matchText(q, `${r.name} ${r.sku} ${r.notes || ""}`));
  ...
  const ceramitec = useMemo(() => CERAMITEC_BUNDLE_SKUS.map(sku => ({
    sku,
    p: (products || []).find(x => normSku(x.sku) === normSku(sku))
  })).filter(({
    sku,
    p
  }) => matchText(q, `${p ? p.name : ""} ${sku}`)), [products, q]);
  const refresh = async () => {
    setRefreshing(true);
    try {
  ...
  const header = /*#__PURE__*/React.createElement("div", {
    ...
    value: "all"
  }, "All vendors"), vendors.map(([v, n]) => /*#__PURE__*/React.createElement("option", {
    key: v,
    value: v
  ...
  }, [["below", `Below reorder level (${counts.below})`], ["short", `Needs ordering (${counts.short})`], ["restocked", `Restocked (${counts.restocked})`], ["all", `All (${counts.all})`], ["ceramitec", `Ceramitec bundles (${CERAMITEC_BUNDLE_SKUS.length})`]].map(([k, label]) => /*#__PURE__*/React.createElement("button", {
    key: k,
    onClick: () => setView(k),
    style: {
      ...catPill(view === k),
  ...
  if (view === "ceramitec") return /*#__PURE__*/React.createElement("div", null, header, /*#__PURE__*/React.createElement(CeramitecBundles, {
    rows: ceramitec,
    manual: doc.manual,
    onSave: saveManual
  }));
  if (!doc.lastSyncAt) return /*#__PURE__*/React.createElement("div", null, header, /*#__PURE__*/React.createElement("div", {
    style: {
      textAlign: "center",
      padding: "60px 20px",
      background: "var(--card)",
  ...
  }, "Not pulled from Zoho yet"), /*#__PURE__*/React.createElement("p", {
    style: {
      color: "var(--muted)",
      maxWidth: 440,
      margin: "0 auto",
...
function CeramitecBundles({
  ...
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
  ...
  }, "Stock on hand"), /*#__PURE__*/React.createElement("div", {
    style: {
      textAlign: "center"
    }
  }, "To be received"), /*#__PURE__*/React.createElement("div", null, "Expected by"), /*#__PURE__*/React.createElement("div", null, "Notes")), !rows.length && /*#__PURE__*/React.createElement("div", {
    style: {
      padding: "30px 16px",
      textAlign: "center",
      color: "var(--muted)",
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      style: {
        fontWeight: 600,
        color: p ? "var(--ink)" : "var(--muted)"
      }
    }, p ? p.name : "Not in the catalog yet — run Sync with Shopify"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 11.5,
        color: "var(--muted)"
      }
      ...
      style: {
        ...midCell,
        fontWeight: 600,
        color: !p || !p.stockTracked ? "var(--muted)" : (p.stock || 0) <= 0 ? "var(--clay)" : p.stock < 10 ? "var(--amber)" : "var(--sage)"
      }
    }, !p ? "—" : p.stockTracked ? p.stock || 0 : "not tracked"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        justifyContent: "center"
      }
    }, /*#__PURE__*/React.createElement(QtyCell, {
      value: m.toReceive,
      onSave: v => onSave(sku, {
        toReceive: v
      })
    })), /*#__PURE__*/React.createElement(ExpectedDateCell, {
      value: m.expectedDate || "",
      onChange: v => onSave(sku, {
        expectedDate: v
      })
  ...
  }, "Pre-packed Ceramitec sets, sold as single products on Shopify. Stock on hand is what's entered on Shopify (updated on every Shopify sync); to be received, dates and notes are filled in here."));
}
...
const CERAMITEC_BUNDLE_SKUS = ["CER04TS063",
// Morya Table Setting (4 Pieces)
"CER13TS284",
// Rangoli Table Setting Pink (13 Pieces)
"CER083TS104",
// Rangoli Table Settings Lilac (13 pieces)
"CER04TS068",
// Raya Table Setting (4 Pieces)
"CER05TS047",
// Heart Beat Table Setting Pink (5 Pieces)
"CER082TS105",
// Bites and Delights Lime Green
...
function InventoryApp() {
  ...
  useEffect(() => {
    ...
    fetch("/api/data").then(r => r.json()).then(d => setProducts(d && d.products || [])).catch(() => {});
  }, []);
  useEffect(() => {
