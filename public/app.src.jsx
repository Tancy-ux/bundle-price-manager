...
function Reorder({ doc, setDoc, stickyTop, flash }) {
  ...
  const cooldownLeft = doc.lastManualSyncAt
    ? REORDER_COOLDOWN_MS - (Date.now() - new Date(doc.lastManualSyncAt).getTime())
    : 0;
  ...
  const refresh = async () => {
    ...
    try {
      ...
      if (r.status === 429) {
        flash(
          `Refresh was used recently — try again in ${fmtLeft(j.retryAfterMs)}`,
        );
      } else if (!j.ok) {
    ...
    >
      ...
      >
        ...
        <button
          ...
          title={
            coolingDown
              ? `Updated ${timeAgo(doc.lastSyncAt)}. The button can be used once every 8h — available again in ${fmtLeft(cooldownLeft)}. It also refreshes on its own every 8h.`
        ...
        >
          {refreshing
            ? "Refreshing…"
            : coolingDown
              ? `Updated ${timeAgo(doc.lastSyncAt)} · refresh again in ${fmtLeft(cooldownLeft)}`
              : doc.lastSyncAt
  ...
  return (
    <div>
      ...
      <p style={{ ...note, marginTop: 10 }}>
        ...
        Inventory (also refreshed automatically every 8 hours). "To be received"
...
function InventoryApp() {
  ...
  return (
    <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 20px 60px" }}>
      <div
        ref={headerRef}
        style={{ position: "sticky", top: 0, zIndex: 20, background: "var(--paper)", padding: "26px 0 14px" }}
      >
        <div style={{ fontSize: 11, letterSpacing: 3, textTransform: "uppercase", color: "var(--clay)", fontWeight: 700 }}>
          Ware Innovations
        </div>
        <h1 className="serif" style={{ fontSize: 32, margin: "6px 0 0", fontWeight: 600, letterSpacing: -0.5 }}>
          Zoho Inventory
        </h1>
      </div>
      {!doc ? (
...
ReactDOM.createRoot(document.getElementById("root")).render(
  window.APP_VIEW === "inventory" ? <InventoryApp /> : <App />,
);
