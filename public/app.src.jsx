...
function Products({
  ...
  const counts = useMemo(() => {
    ...
    return { all: normalProducts.length, used, unused, oos, atelier: atelierProducts.length };
  }, [normalProducts, atelierProducts, usage]);
  // bulk: deactivate every currently-shown product that isn't in any bundle (with undo)
  const unusedShownActive = filtered.filter(
    (p) => usedCount(p) === 0 && p.active,
  );
  const archiveUnused = () => {
    const ids = new Set(unusedShownActive.map((p) => p.id));
    if (!ids.size) return;
    setProducts(
      products.map((p) => (ids.has(p.id) ? { ...p, active: false } : p)),
    );
    showUndo(
      `Archived ${ids.size} unused product${ids.size > 1 ? "s" : ""}`,
      () =>
        setProducts((cur) =>
          cur.map((p) => (ids.has(p.id) ? { ...p, active: true } : p)),
        ),
    );
  };
      ...
      >
        ...
        >
          ...
          {unusedShownActive.length > 0 && (
            ...
            >
              Archive {unusedShownActive.length} unused →
            </button>
          )}
        </div>
      </div>
