...
async function apiGet(){ const r = await fetch("/api/data"); return r.json(); }
async function apiPut(products,bundles,trash,excludedSkus,excludedBundleNames){
  const r = await fetch("/api/data",{method:"PUT",headers:{"Content-Type":"application/json"},
    body:JSON.stringify({products,bundles,trash:trash||[],excludedSkus:excludedSkus||[],excludedBundleNames:excludedBundleNames||[]})});
  return r.json();
}
...
function App(){
  ...
  useEffect(()=>{(async()=>{
    ...
    setExcludedSkus(d.excludedSkus||[]); setExcludedBundleNames(d.excludedBundleNames||[]);
  ...
  useEffect(()=>{
    ...
    let cancel=false;
    setSaving(true);
    apiPut(products,bundles,trash,excludedSkus,excludedBundleNames).then(()=>{ if(!cancel){ setSaving(false);} })
      .catch(()=>{ if(!cancel){ setSaving(false); flash("Could not save to disk — is the server running?"); }});
    return ()=>{cancel=true;};
  ...
  const excludeSku=(sku)=>{ const s=normSku(sku); if(!s) return; setExcludedSkus(cur=>cur.includes(s)?cur:[...cur,s]); };
  const unexcludeSku=(sku)=>{ const s=normSku(sku); setExcludedSkus(cur=>cur.filter(x=>x!==s)); };
  const excludeBundleName=(name)=>{ const n=norm(name); if(!n) return; setExcludedBundleNames(cur=>cur.includes(n)?cur:[...cur,n]); };
  const unexcludeBundleName=(name)=>{ const n=norm(name); setExcludedBundleNames(cur=>cur.filter(x=>x!==n)); };
  ...
  function deleteBundle(b){
    ...
    setTrash(t=>[entry,...t]);
    showUndo(`Deleted "${b.name}"`, ()=>{ setBundles(cur=>[b,...cur]); setTrash(t=>t.filter(e=>e!==entry)); unexcludeBundleName(b.name); });
  }
  ...
  function deleteProduct(p){
    ...
    setTrash(t=>[entry,...t]);
    showUndo(`Deleted "${p.name}"`, ()=>{ setProducts(cur=>[p,...cur]); setTrash(t=>t.filter(e=>e!==entry));
      if(p.sku) unexcludeSku(p.sku); else unexcludeBundleName(p.name); });
  }
  function restoreFromTrash(entry){
    if(entry.kind==="bundle"){ setBundles(cur=>[entry.item,...cur]); unexcludeBundleName(entry.item.name); }
    else { setProducts(cur=>[entry.item,...cur]);
  ...
  function promoteToProduct(b){
    const c=compute(b);
    const np={id:uid(),sku:"",name:b.name,price:c.empty?(b.storedPrice||0):c.target,active:true};
  ...
  function skipBundle(b,skip){
    setBundles(cur=>cur.map(x=>x.id===b.id?{...x,skipped:skip}:x));
    if(skip) showUndo(`Skipped "${b.name}"`, ()=> setBundles(cur=>cur.map(x=>x.id===b.id?{...x,skipped:false}:x)));
    else flash(`"${b.name}" back in the worklist`);
  }

  const byId = useMemo(()=>{ const m={}; (products||[]).forEach(p=>m[p.id]=p); return m; },[products]);
  ...
  const bundleNameCounts = useMemo(()=>{ const m={};
    (bundles||[]).forEach(b=>{ const n=norm(b.name); if(n) m[n]=(m[n]||0)+1; }); return m; },[bundles]);
  ...
  function compute(b){
    ...
    return {target,missing,stale,empty,giftIncluded,skipped,priceMismatch,diff:target-(b.storedPrice||0),oosItems,hasOOS,stock,dupName};
  }
  ...
  return (
    <div style={{maxWidth:880,margin:"0 auto",padding:"0 20px 60px"}}>
      <div style={{position:"sticky",top:0,zIndex:15,background:"var(--paper)",paddingTop:26,
      ...
      {toast && <div style={{position:"fixed",bottom:18,left:"50%",transform:"translateX(-50%)",zIndex:30,
        background:"var(--ink)",color:"var(--paper)",padding:"10px 16px",borderRadius:8,fontSize:13,fontWeight:600,
        animation:"slideIn .2s ease"}}>{toast}</div>}
      {undo && <div style={{position:"fixed",bottom:18,left:"50%",transform:"translateX(-50%)",zIndex:31,
        background:"var(--ink)",color:"var(--paper)",padding:"8px 10px 8px 16px",borderRadius:10,fontSize:13,
        display:"flex",alignItems:"center",gap:14,animation:"slideIn .2s ease"}}>
...
function Worklist({staleList,bundles,setBundles,flash,products,compute,byId,onDelete,onPromote,onSkip}){
  ...
  const Tabs=(
    <div style={{display:"flex",gap:6,marginBottom:14}}>
      {[["todo",`To update${staleList.length?` (${staleList.length})`:""}`],["history",`History${history.length?` (${history.length})`:""}`]].map(([k,label])=>(
        <button key={k} onClick={()=>setView(k)} style={{padding:"6px 12px",borderRadius:999,fontSize:12.5,fontWeight:600,cursor:"pointer",
          border:`1px solid ${view===k?"var(--clay)":"var(--line)"}`,
          background:view===k?"var(--clayDim)":"#fff",color:view===k?"var(--clay)":"var(--muted)"}}>{label}</button>
      ))}
    </div>
  );
  ...
  if(!staleList.length) return (
    <div>
      ...
      <div style={{textAlign:"center",padding:"60px 20px",background:"var(--card)",border:"1px solid var(--line)",borderRadius:14}}>
        ...
        <h2 className="serif" style={{fontSize:22,margin:"0 0 6px"}}>Everything's in sync</h2>
        <p style={{color:"var(--muted)",maxWidth:380,margin:"0 auto",lineHeight:1.5}}>Change a product price and the affected bundles appear here with corrected prices.</p>
      </div>
    </div>
  );
  const broken=staleList.filter(x=>x.c.missing), priced=staleList.filter(x=>!x.c.missing);
  return (
    <div>
      {Tabs}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,gap:10}}>
        <p style={{margin:0,fontSize:14}}>{staleList.length} bundle{staleList.length>1?"s":""} out of sync.</p>
        <div style={{display:"flex",gap:8}}>
          <button onClick={copyAll} style={btnSec}>Copy all</button>
          <button onClick={fixAll} style={btnPri}>Mark all updated</button>
        </div>
      </div>
      {broken.length>0 && <div style={{background:"var(--clayDim)",color:"var(--clay)",padding:"10px 14px",
        borderRadius:8,fontSize:13,fontWeight:600,marginBottom:12}}>⚠ {broken.length} bundle(s) use an inactive/missing product — fix in Bundles.</div>}
      <div style={{background:"var(--card)",border:"1px solid var(--line)",borderRadius:12,overflow:"hidden"}}>
        <div style={{...wlGrid,padding:"10px 16px",fontSize:11,textTransform:"uppercase",letterSpacing:.5,
          color:"var(--muted)",fontWeight:700,borderBottom:"1px solid var(--line)"}}>
          <span>Bundle</span><span style={{textAlign:"right"}}>Live now</span>
          <span style={{textAlign:"right"}}>Should be</span><span style={{textAlign:"right"}}>Change</span><span/>
        </div>
        {priced.map(({b,c})=>{ const open=openId===b.id; return (
          <div key={b.id} style={{borderBottom:"1px solid var(--line)"}}>
            <div style={{...wlGrid,padding:"11px 16px",alignItems:"center"}}>
              <button onClick={()=>setOpenId(open?null:b.id)} style={{display:"flex",alignItems:"center",gap:8,
                background:"none",border:"none",cursor:"pointer",textAlign:"left",padding:0,font:"inherit"}}>
                <span style={{color:"var(--muted)",fontSize:12,width:10}}>{open?"▾":"▸"}</span>
                <span><span style={{fontSize:14,fontWeight:600}}>{b.name}</span>
                  {b.sku && <span style={{fontSize:11.5,color:"var(--muted)",marginLeft:6}}>{b.sku}</span>}</span>
              </button>
              <span style={numCell}>{money(b.storedPrice)}</span>
              <span style={{...numCell,fontWeight:700}}>{money(c.target)}</span>
              <span style={{...numCell,color:c.diff>0?"var(--sage)":"var(--clay)"}}>{c.diff>0?"+":""}{money(c.diff)}</span>
              <button onClick={()=>fixOne(b,c.target)} style={btnFix}>Mark done</button>
            </div>
            {open && (
              <div style={{background:"var(--paper)",borderTop:"1px dashed var(--line)",paddingTop:12}}>
        ...
        {broken.map(({b})=>{ const open=openId===b.id; return (
          <div key={b.id} style={{borderBottom:"1px solid var(--line)"}}>
            <div style={{...wlGrid,padding:"11px 16px",alignItems:"center",opacity:.85}}>
              <button onClick={()=>setOpenId(open?null:b.id)} style={{display:"flex",alignItems:"center",gap:8,
                background:"none",border:"none",cursor:"pointer",textAlign:"left",padding:0,font:"inherit"}}>
                <span style={{color:"var(--muted)",fontSize:12,width:10}}>{open?"▾":"▸"}</span>
                <span style={{fontSize:14,fontWeight:600}}>{b.name}</span>
              </button>
              <span style={{gridColumn:"2 / 6",color:"var(--clay)",fontSize:13,fontWeight:600,textAlign:"right"}}>Has a missing/inactive item — click to fix</span>
            </div>
            {open && (
              <div style={{background:"var(--paper)",borderTop:"1px dashed var(--line)",paddingTop:12}}>
                <BundleEditor b={b} update={update} byId={byId} products={products}
...
function Bundles({bundles,setBundles,products,compute,byId,onDelete,onPromote,onSkip}){
  ...
  const catCounts = useMemo(()=>{
    ...
    const m={}; CATS.forEach(w=>{ m[w]=base.filter(b=>b.name.toLowerCase().includes(w)).length; });
    return {all:base.length, ...m};
  },[bundles,show]);
  const update=(id,patch)=> setBundles(bundles.map(b=>b.id===id?{...b,...patch}:b));
  const add=()=>{ const nb={id:uid(),sku:"",name:"New bundle",items:[],storedPrice:0};
    setBundles([nb,...bundles]); setOpenId(nb.id); setQ(""); };

  return (
    <div>
      <div style={{display:"flex",gap:10,marginBottom:8}}>
        <input placeholder="Search bundles…" value={q} onChange={e=>setQ(e.target.value)} style={search}/>
        <button onClick={add} style={btnPri}>+ New</button>
      </div>
      <div style={{display:"flex",gap:6,marginBottom:8}}>
        {[["all",`All (${bundles.length})`],["empty",`Not built yet (${emptyCount})`],["filled",`Built (${bundles.length-emptyCount})`],["oos",`Out of stock (${oosCount})`],["dup",`Duplicate names (${dupCount})`]].map(([k,label])=>(
      ...
      <div style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap"}}>
        <button onClick={()=>setCat("")} style={catPill(cat==="")}>All types ({catCounts.all})</button>
        {CATS.map(w=>{ const n=catCounts[w]||0;
          ...
          return (<button key={w} onClick={()=>setCat(cat===w?"":w)} style={catPill(cat===w)}>
            {w} ({n})</button>);
        })}
      </div>
      <p style={note}>{filtered.length} match{filtered.length===1?"":"es"}{filtered.length>shown.length?` · showing ${shown.length}`:""}</p>
      {filtered.length===0 && (
        <div style={{textAlign:"center",padding:"36px 20px",background:"var(--card)",border:"1px dashed var(--line)",borderRadius:12,color:"var(--muted)"}}>
          <div style={{fontSize:14,fontWeight:600,color:"var(--ink)",marginBottom:4}}>No bundles match these filters</div>
          <div style={{fontSize:13,marginBottom:14}}>Try fewer words, or clear the filters to see everything.</div>
          <button onClick={()=>{setQ("");setShow("all");setCat("");}} style={btnSec}>Clear filters</button>
        </div>
      )}
      <div style={{display:"flex",flexDirection:"column",gap:6}}>
        {shown.map(b=>{ const c=compute(b); const open=openId===b.id;
          return (
          <div key={b.id} style={{background:"var(--card)",border:`1px solid ${c.stale?"var(--clay)":"var(--line)"}`,
            borderLeft:`3px solid ${c.stale?"var(--clay)":"var(--sage)"}`,borderRadius:10,overflow:"hidden"}}>
            <button onClick={()=>setOpenId(open?null:b.id)} style={{width:"100%",display:"flex",alignItems:"center",
              ...
              <span style={{flex:1,fontSize:14.5,fontWeight:600}}>{b.name}
                {c.dupName && <span title="Another bundle has this exact same name — ambiguous for matching/syncing. Rename one of them." style={{marginLeft:7,fontSize:11,fontWeight:700,
                  color:"var(--amber)",border:"1px solid var(--amber)",borderRadius:999,padding:"1px 7px",verticalAlign:"middle"}}>dup name</span>}
      ...
      {filtered.length>shown.length && (
        <div style={{display:"flex",justifyContent:"center",marginTop:14}}>
          <button onClick={()=>setVisible(v=>v+60)} style={btnSec}>
            Load more ({filtered.length-shown.length} more)</button>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{padding:"0 14px 14px",display:"flex",flexDirection:"column",gap:12}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        <label style={lbl}>Name<input value={b.name} onChange={e=>update(b.id,{name:e.target.value})} style={inp}/></label>
        <label style={lbl}>Bundle SKU<input value={b.sku} onChange={e=>update(b.id,{sku:e.target.value})} style={inp}/></label>
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:6}}>
        <QuickAdd products={products} byId={byId}
          onAdd={(productId,qty)=>{
            const existing = b.items.findIndex(it=>it.productId===productId);
            let items;
            if(existing>=0){ items=[...b.items]; items[existing]={...items[existing],qty:items[existing].qty+qty}; }
      ...
      <div style={{background:c.stale?"var(--clayDim)":"var(--sageDim)",borderRadius:9,padding:"12px 14px",
        ...
        {!c.missing && c.priceMismatch && (
          <label style={{display:"flex",alignItems:"flex-start",gap:8,fontSize:13,cursor:"pointer",
            ...
            <input type="checkbox" checked={!!b.giftIncluded}
              onChange={e=>update(b.id,{giftIncluded:e.target.checked})}
              style={{marginTop:2,cursor:"pointer"}}/>
            <span>Gift / packaging price included — the live price is meant to differ from the item sum, so don't flag this for updating.</span>
          </label>
        )}
      ...
      <label style={{...lbl,marginTop:2}}>Note (optional)
        <textarea value={b.note||""} onChange={e=>update(b.id,{note:e.target.value})}
          placeholder="e.g. includes gift box (+₹150) · seasonal pricing · anything worth remembering"
          rows={2}
          style={{...inp,resize:"vertical",fontFamily:"inherit",lineHeight:1.4}}/></label>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,
        background:b.skipped?"var(--sageDim)":"transparent",borderRadius:8,
        padding:b.skipped?"8px 12px":"0"}}>
        <div style={{fontSize:12.5,color:"var(--muted)",lineHeight:1.4}}>
          {b.skipped
            ? <span style={{color:"var(--sage)",fontWeight:600}}>Skipped — kept out of “Needs updating”. Won't be flagged even if the price differs.</span>
            : <span>Just one item + a box, or otherwise not worth syncing? Skip it to keep it off the worklist.</span>}
        </div>
        <button onClick={()=>onSkip(b,!b.skipped)} style={{...btnSec,whiteSpace:"nowrap",
          ...(b.skipped?{color:"var(--sage)",borderColor:"var(--sage)"}:{})}}>
          {b.skipped?"Un-skip":"Skip this bundle"}</button>
      </div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",borderTop:"1px solid var(--line)",paddingTop:10,marginTop:2}}>
        <button onClick={()=>onPromote(b)} style={{background:"none",border:"1px solid var(--line)",color:"var(--muted)",
          padding:"6px 12px",borderRadius:7,fontSize:12.5,fontWeight:600,cursor:"pointer"}}
          title="This isn't a bundle — move it into the product list">Not a bundle → move to Products</button>
        <button onClick={()=>onDelete(b)} style={{background:"none",border:"none",color:"var(--clay)",
          padding:"6px 10px",borderRadius:7,fontSize:12.5,fontWeight:600,cursor:"pointer"}}>Delete bundle</button>
      </div>
    </div>
  );
}
...
function QuickAdd({products,byId,onAdd}){
  ...
  useEffect(()=>{ setHi(0); setOpen(matches.length>0); },[q]);
  ...
  function onKey(e){
    if(!open && (e.key==="ArrowDown")) { setOpen(matches.length>0); return; }
    ...
    <div style={{position:"relative",marginBottom:4}}>
      <div style={{display:"flex",gap:8,alignItems:"center"}}>
        <input type="number" min="1" value={qty} onChange={e=>setQty(e.target.value)}
          title="quantity"
          style={{width:52,padding:"9px 8px",borderRadius:8,border:"1px solid var(--line)",fontSize:14,textAlign:"center"}}/>
        <span style={{color:"var(--muted)",fontSize:13}}>×</span>
        <input ref={inputRef} value={q} onChange={e=>setQ(e.target.value)} onKeyDown={onKey}
          onFocus={()=>setOpen(matches.length>0)}
          placeholder="type to add a component — e.g. lunar nude plate, then Enter"
          style={{flex:1,padding:"9px 12px",borderRadius:8,border:"1px solid var(--clay)",fontSize:14,background:"#fff"}}/>
      </div>
      {open && matches.length>0 && (
        <div style={{position:"absolute",top:"100%",left:60,right:0,zIndex:30,marginTop:4,background:"#fff",
          border:"1px solid var(--line)",borderRadius:10,boxShadow:"0 8px 24px rgba(0,0,0,.12)",overflow:"hidden"}}>
          {matches.map((p,i)=>(
            <div key={p.id} onMouseDown={(e)=>{e.preventDefault(); commit(p);}}
              onMouseEnter={()=>setHi(i)}
              style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,padding:"9px 12px",
                cursor:"pointer",background: i===hi?"var(--sageDim)":"#fff",borderBottom:"1px solid var(--line)"}}>
              <span style={{fontSize:13.5}}>{p.name}</span>
              <span style={{fontSize:12,color:"var(--muted)",whiteSpace:"nowrap"}}>{p.sku} · {money(p.price)}</span>
            </div>
          ))}
        </div>
      )}
...
function Products({products,setProducts,bundles,onDelete,showUndo,flash}){
  ...
  const filtered = useMemo(()=>{ if(!q.trim()) return baseList;
    return baseList.filter(p=>matchText(q, p.name+" "+(p.sku||""))); },[baseList,q]);
  useEffect(()=>{ setVisible(100); },[q,usageFilter]);
  const shown = filtered.slice(0,visible);
  const update=(id,patch)=> setProducts(products.map(p=>p.id===id?{...p,...patch}:p));
  const add=()=>{ const np={id:uid(),sku:"",name:"New product",price:0,active:true}; setProducts([np,...products]); setQ(""); };
  ...
  return (
    <div>
      <div style={{display:"flex",gap:10,marginBottom:8}}>
        <input placeholder="Search products…" value={q} onChange={e=>setQ(e.target.value)} style={search}/>
        <button onClick={add} style={btnPri}>+ New</button>
      </div>
      <div style={{display:"flex",gap:6,marginBottom:8,flexWrap:"wrap",alignItems:"center"}}>
        ...
        {unusedShownActive.length>0 && (
          <button onClick={archiveUnused} style={{...btnSec,marginLeft:"auto",color:"var(--clay)",borderColor:"var(--clayDim)"}}
            title="Set every active product here that isn't used in any bundle to Inactive">
            Archive {unusedShownActive.length} unused →</button>
        )}
      </div>
      <p style={note}>{filtered.length} match{filtered.length===1?"":"es"}{filtered.length>shown.length?` · showing first ${shown.length}`:""} · edit a price and every bundle using it updates</p>
      {filtered.length===0 && (
        <div style={{textAlign:"center",padding:"36px 20px",background:"var(--card)",border:"1px dashed var(--line)",borderRadius:12,color:"var(--muted)",marginBottom:8}}>
          <div style={{fontSize:14,fontWeight:600,color:"var(--ink)",marginBottom:4}}>{q.trim()?`No products match “${q}”`:"No products in this view"}</div>
          <div style={{fontSize:13,marginBottom:14}}>{usageFilter!=="all"?"Try the All filter, or ":"Try fewer words, or "}clear to see all {products.length} products.</div>
          <button onClick={()=>{setQ("");setUsageFilter("all");}} style={btnSec}>Clear filters</button>
        </div>
      )}
      <div style={{background:"var(--card)",border:"1px solid var(--line)",borderRadius:12,overflow:"hidden"}}>
        ...
        {shown.map(p=>{ const used=usage[p.id]||[]; const isOpen=expanded===p.id; return (
          <div key={p.id} style={{borderBottom:"1px solid var(--line)",opacity:p.active?1:.5}}>
            <div style={{...prodGrid,padding:"7px 14px",alignItems:"center"}}>
              ...
              {used.length>0
                ? <button onClick={()=>setExpanded(isOpen?null:p.id)}
                    ...
                    style={{textAlign:"center",fontSize:13,color:"var(--amber)",fontWeight:700,background:"none",
                      border:"none",cursor:"pointer",textDecoration:"underline",textUnderlineOffset:2}}>
                    {used.length} {isOpen?"▾":"▸"}</button>
                : <span style={{textAlign:"center",fontSize:13,color:"var(--muted)"}}>—</span>}
              <button onClick={()=>update(p.id,{active:!p.active})} style={{border:`1px solid ${p.active?"var(--sage)":"var(--line)"}`,
                background:p.active?"var(--sageDim)":"#fff",color:p.active?"var(--sage)":"var(--muted)",borderRadius:999,
                padding:"5px 0",fontSize:12,fontWeight:600,cursor:"pointer"}}>{p.active?"Active":"Inactive"}</button>
              <button onClick={()=>onDelete(p)} style={xBtn}>✕</button>
            </div>
            {isOpen && used.length>0 && (
              <div style={{padding:"4px 14px 12px 14px",display:"flex",flexWrap:"wrap",gap:6}}>
                {used.map((u,i)=>(
                  <span key={i} style={{fontSize:12.5,background:"var(--sageDim)",color:"var(--ink)",
                    borderRadius:999,padding:"3px 10px",border:"1px solid var(--line)"}}>
                    {u.name}{u.sku?` · ${u.sku}`:""}</span>
                ))}
              </div>
            )}
          </div>
...
function WhereUsed({bundles,products,compute}){
  ...
  const results = useMemo(()=>{
    ...
    return products
      ...
      .map(p=>({p, used: bundles.filter(b=>b.items.some(it=>it.productId===p.id))}))
      .filter(x=>x.used.length>0)
      .sort((a,b)=>b.used.length-a.used.length);
  },[q,products,bundles]);
  let totalBundles=0;
  if(results){ const s=new Set(); results.forEach(x=>x.used.forEach(b=>s.add(b.id))); totalBundles=s.size; }

  return (
    <div>
      <input placeholder="Search a product — e.g. dot pink — to see its bundles…" value={q}
        onChange={e=>setQ(e.target.value)} style={search}/>
      {!q.trim() && (
        <p style={{...note,marginTop:14}}>Type a product name or SKU. Every bundle that contains a matching
          product is listed below. Products that aren't in any bundle won't show up here.</p>
      )}
      {results && results.length===0 && (
        <div style={{textAlign:"center",padding:"36px 20px",background:"var(--card)",border:"1px dashed var(--line)",
          borderRadius:12,color:"var(--muted)",marginTop:14}}>
          <div style={{fontSize:14,fontWeight:600,color:"var(--ink)",marginBottom:4}}>No bundle uses a product matching “{q}”</div>
          <div style={{fontSize:13}}>The product may exist but isn't a component of any bundle yet.</div>
        </div>
      )}
      {results && results.length>0 && (
        <p style={{...note,marginTop:14}}>{results.length} matching product{results.length>1?"s":""} · {totalBundles} bundle{totalBundles>1?"s":""}</p>
      )}
      <div style={{display:"flex",flexDirection:"column",gap:16}}>
        {results && results.map(({p,used})=>(
          <div key={p.id} style={{background:"var(--card)",border:"1px solid var(--line)",borderRadius:12,overflow:"hidden"}}>
            <div style={{display:"flex",alignItems:"baseline",gap:8,padding:"11px 16px",borderBottom:"1px solid var(--line)"}}>
              <span style={{fontSize:14.5,fontWeight:700}}>{p.name}</span>
              {p.sku && <span style={{fontSize:11.5,color:"var(--muted)"}}>{p.sku}</span>}
              <span style={{marginLeft:"auto",fontSize:12.5,color:"var(--muted)"}}>in {used.length} bundle{used.length>1?"s":""}</span>
            </div>
            {used.map(b=>{ const c=compute(b); const it=b.items.find(i=>i.productId===p.id);
              return (
                <div key={b.id} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 16px",borderBottom:"1px solid var(--line)"}}>
...
function Trash({trash,onRestore,onDelete,onEmpty}){
  if(!trash.length) return (
    <div style={{textAlign:"center",padding:"60px 20px",background:"var(--card)",border:"1px solid var(--line)",borderRadius:14}}>
      <h2 className="serif" style={{fontSize:22,margin:"0 0 6px"}}>Trash is empty</h2>
      <p style={{color:"var(--muted)",maxWidth:380,margin:"0 auto",lineHeight:1.5}}>Deleted bundles and products land here. You can restore them anytime, or empty the trash to remove them for good.</p>
    </div>
  );
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        <p style={{margin:0,fontSize:14}}>{trash.length} deleted item{trash.length>1?"s":""}.</p>
        <button onClick={onEmpty} style={{...btnSec,color:"var(--clay)",borderColor:"var(--clayDim)"}}>Empty trash</button>
      </div>
      <div style={{background:"var(--card)",border:"1px solid var(--line)",borderRadius:12,overflow:"hidden"}}>
        {trash.map((e,i)=>(
          <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,
            ...
            <div>
              ...
              <div style={{fontSize:11.5,color:"var(--muted)"}}>{e.kind} · deleted {new Date(e.at).toLocaleDateString()}</div>
            </div>
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>onRestore(e)} style={btnSec}>Restore</button>
              <button onClick={()=>onDelete(e)} style={{...btnSec,color:"var(--clay)",borderColor:"var(--clayDim)"}}>Delete</button>
            </div>
          </div>
        ))}
      </div>
...
const btnPri={background:"var(--ink)",color:"var(--paper)",border:"none",padding:"9px 15px",borderRadius:8,fontSize:13,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap"};
const catPill=(on)=>({padding:"4px 11px",borderRadius:999,fontSize:12,fontWeight:600,cursor:"pointer",textTransform:"capitalize",
  border:`1px solid ${on?"var(--sage)":"var(--line)"}`,background:on?"var(--sageDim)":"#fff",color:on?"var(--sage)":"var(--muted)"});
const btnSec={background:"#fff",color:"var(--ink)",border:"1px solid var(--line)",padding:"9px 15px",borderRadius:8,fontSize:13,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap"};
const btnFix={background:"var(--clay)",color:"#fff",border:"none",padding:"7px 10px",borderRadius:7,fontSize:12.5,fontWeight:600,cursor:"pointer"};
const xBtn={background:"none",border:"none",color:"var(--muted)",cursor:"pointer",fontSize:13,padding:4};
const search={flex:1,padding:"9px 12px",borderRadius:8,border:"1px solid var(--line)",fontSize:14,background:"#fff"};
const note={fontSize:12.5,color:"var(--muted)",margin:"0 0 14px"};
const lbl={display:"flex",flexDirection:"column",gap:4,fontSize:11,color:"var(--muted)",fontWeight:600};
const inp={padding:"8px 10px",borderRadius:7,border:"1px solid var(--line)",fontSize:14,fontWeight:400,color:"var(--ink)"};
const sel={padding:"7px 9px",borderRadius:7,border:"1px solid var(--line)",background:"#fff",fontSize:13};
