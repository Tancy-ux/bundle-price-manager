...
async function apiPut(products,bundles,trash){
  const r = await fetch("/api/data",{method:"PUT",headers:{"Content-Type":"application/json"},
    body:JSON.stringify({products,bundles,trash:trash||[]})});
  return r.json();
}
...
function App(){
  ...
  useEffect(()=>{
    ...
    let cancel=false;
    setSaving(true);
    apiPut(products,bundles,trash).then(()=>{ if(!cancel){ setSaving(false);} })
  ...
  function restoreFromTrash(entry){
    if(entry.kind==="bundle") setBundles(cur=>[entry.item,...cur]);
